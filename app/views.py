...
@login_required
def paymentsuccessfulwhatsapp(request):
    totalitem = 0
    if request.user.is_authenticated:
        totalitem = len(Cart.objects.filter(user=request.user))
    paid = Paywhatsapp.request.GET('')
    return render(request, 'app/paymentsuccessfulwhatsapp.html', locals())
...
def PayApp(request):
    ...
    if request.method == 'POST':
        ...
        paid.city = request.POST.get('city')
        paid.email = request.POST.get('email')
...
def Paywhatsapp(request):
    ...
    cart_items = Cart.objects.filter(user = user)
    famount = 0
    dynamic_message = "Hello here's my order details: \n\n "
    for p in cart_items:
        quantity = p.quantity
        ...
        dynamic_message += f"Product : {product}\n"
        dynamic_message += f"Quantity : {quantity}\n"
        dynamic_message += f"Price per Unit : {price}\n"
        dynamic_message += f"Subtotal : {value}\n\n"
    totalamount = famount
    dynamic_message += f"Total : {totalamount}\n\n Payment Method : Whatsapp contact \n\n Reciever Details:\n"
    ...
    if request.method == 'POST':
        ...
        if not paid.name or not paid.contact or not paid.address or not paid.state:
            messages.info(request, 'All Fields must be filled out.')
            return redirect('paywhatsapp')
        paid.save()
        
        dynamic_message += f"Name: {paid.name}\n"
        dynamic_message += f"Phone: {paid.contact}\n"
        dynamic_message += f"Location: {paid.address}, "
        dynamic_message += f"{paid.city}, "
        dynamic_message += f"{paid.state}\n\n"
        dynamic_message += f"Waybill is Free!! \n\n Thank You!"

        for c in cart_items:
            OrderPlaced(user=user, product=c.product, quantity = c.quantity, payment=paid).save()
            c.delete()
        whatsapp_url = f"https://wa.me/2349021865092?text={dynamic_message}"
        return redirect(whatsapp_url)
    return render(request, 'app/paywhatsapp.html', locals())
